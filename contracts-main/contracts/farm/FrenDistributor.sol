...
contract FrenDistributor is Ownable, ReentrancyGuard {
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accFrenPerShare: 0, depositFeeBP: _depositFeeBP, harvestInterval: _harvestInterval, totalLp: 0}));
    }
    ...
    function pendingFren(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accFrenPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 frenReward =
            multiplier.mul(frenPerBlock).mul(pool.allocPoint).div(
                totalAllocPoint
            );
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(block.number >= startBlock, "FrenDistributor: Can not deposit before start");
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(fren)) {
            totalFrenInPools = totalFrenInPools.sub(amount);
        }
    ...
    function safeFrenTransfer(address _to, uint256 _amount) internal {
        if (fren.balanceOf(address(this)) > totalFrenInPools) {
            //FrenBal = total Fren in FrenDistributor - total Fren in Fren pools, this will make sure that FrenDistributor never transfer rewards from deposited Fren pools
            uint256 FrenBal = fren.balanceOf(address(this)).sub(totalFrenInPools);
            if (_amount >= FrenBal) {
