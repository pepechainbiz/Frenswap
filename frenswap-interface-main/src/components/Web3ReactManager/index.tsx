...
export default function Web3ReactManager({ children }: { children: JSX.Element }) {
  ...
  //   if (router.route !== '/bridge' && chainId !== ChainId.PEPECHAIN_SEPOLIA) {
  ...
  useEffect(() => {
    if (window && window.ethereum && router.route !== '/bridge' && router.route !== '/bridge/history') {
      ...
      const params = SUPPORTED_NETWORKS[ChainId.PEPECHAIN_SEPOLIA]
  ...
  if (wrongNetwork) {
    return (
      <h1 className="text-center">
        ...
        <br />
        {`Switch to Pepechain_Sepolia Network to use Frenswap.club.`}
      </h1>
    )
  }
