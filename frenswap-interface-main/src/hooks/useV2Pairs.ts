...
import { FREN_ADDRESS, FACTORY_ADDRESS, FREN_DISTRIBUTOR_ADDRESS, FREN_VAULT_ADDRESS } from '../constants'
...
export function useVaultTVL(): TVLInfo[] {
  ...
  const farmingPools = Object.keys(VAULTS[ChainId.PEPECHAIN_SEPOLIA]).map((key) => {
    return { ...VAULTS[ChainId.PEPECHAIN_SEPOLIA][key] }
  })
  ...
  const distributorBalance = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'balanceOf', [
    FREN_VAULT_ADDRESS[ChainId.PEPECHAIN_SEPOLIA],
  ])
  ...
  return useMemo(() => {
    function isKnownToken(token: TokenInfo) {
      return (
        token.id.toLowerCase() == FREN_ADDRESS[chainId].toLowerCase() ||
        token.symbol == 'WPEPE' ||
        token.symbol == 'PEPE' ||
        token.symbol == 'RIB' ||
        token.symbol == 'USDC' ||
    ...
    function getPrice(token: TokenInfo) {
      if (token.id.toLowerCase() == FREN_ADDRESS[chainId].toLowerCase()) {
        return frenPrice
      }
      if (token.symbol == 'WPEPE' || token.symbol == 'PEPE') {
        return pepePrice
      }
...
export function useTVL(): TVLInfo[] {
  ...
  const farmingPools = Object.keys(POOLS[ChainId.PEPECHAIN_SEPOLIA]).map((key) => {
    return { ...POOLS[ChainId.PEPECHAIN_SEPOLIA][key], lpToken: key }
  })
  ...
  const totalSupply = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'totalSupply')
  const distributorBalance = useMultipleContractSingleData(pairAddresses, PAIR_INTERFACE, 'balanceOf', [
    FREN_DISTRIBUTOR_ADDRESS[ChainId.PEPECHAIN_SEPOLIA],
  ])
  const distributorBalanceSingle = useMultipleContractSingleData(singleAddresses, PAIR_INTERFACE, 'balanceOf', [
    FREN_DISTRIBUTOR_ADDRESS[ChainId.PEPECHAIN_SEPOLIA],
  ])
...
export function useV2PairsWithPrice(
  ...
  return useMemo(() => {
    function isKnownToken(token: Token) {
      return (
        token.address.toLowerCase() == FREN_ADDRESS[chainId].toLowerCase() ||
        token.symbol == 'WPEPE' ||
        token.symbol == 'PEPE' ||
        token.symbol == 'RIB' ||
        token.symbol == 'USDC' ||
    ...
    function getPrice(token: Token) {
      if (token.address.toLowerCase() == FREN_ADDRESS[chainId].toLowerCase()) {
        return frenPrice
      }
      if (token.symbol == 'WPEPE' || token.symbol == 'PEPE') {
        return pepePrice
  ...
  }, [results, chainId, frenPrice, pepePrice, ribPrice, tokens, totalSupply])
}
