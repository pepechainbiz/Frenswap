...
export default function Bridge() {
  ...
  const [chainTo, setChainTo] = useState<Chain | null>(
    chainId == ChainId.PEPECHAIN_SEPOLIA ? DEFAULT_CHAIN_FROM : DEFAULT_CHAIN_TO
  )
  ...
  const anyswapCurrencyContract = useAnyswapTokenContract(
    currency0 && currency0.chainId == ChainId.PEPECHAIN_SEPOLIA && tokenToBridge.other.ContractAddress,
    true
  )
    ...
    let tokens: Currency[] = Object.keys((anyswapInfo && anyswapInfo[chainFrom.id]) || {})
      ...
      .map((r) => {
        ...
        if (r.toLowerCase() == WNATIVE[chainFrom.id].address.toLowerCase()) {
          if (chainFrom.id == ChainId.PEPECHAIN_SEPOLIA) {
            return Pepechain_Sepolia.onChain(chainFrom.id)
          }
  ...
  const handleChainFrom = useCallback(
    (chain: Chain) => {
      ...
      if (changeTo.id !== ChainId.PEPECHAIN_SEPOLIA && chain.id !== ChainId.PEPECHAIN_SEPOLIA) {
  ...
  const handleChainTo = useCallback(
    (chain: Chain) => {
      ...
      if (changeFrom.id !== ChainId.PEPECHAIN_SEPOLIA && chain.id !== ChainId.PEPECHAIN_SEPOLIA) {
  ...
  const bridgeToken = async () => {
    ...
    const depositAddress = currency0.chainId == ChainId.PEPECHAIN_SEPOLIA ? token.ContractAddress : token.DepositAddress

    const amountToBridge = ethers.utils.parseUnits(currencyAmount, token.Decimals)
    setPendingTx(true)

    try {
      if (currency0.chainId == ChainId.PEPECHAIN_SEPOLIA) {
  ...
  return (
    <>
      ...
      <Head>
        <title>{i18n._(t`Bridge`)} | Frenswap</title>
            ...
            <div className="p-4 text-center">
              <div className="justify-between space-x-3 items-center">
                <Typography component="h3" variant="base">
                  {i18n._(t`Bridge tokens to and from the Pepechain_Sepolia Network`)}
                </Typography>
              </div>
            </div>
