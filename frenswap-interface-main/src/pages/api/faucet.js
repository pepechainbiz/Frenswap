...
async function faucetSend(req) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    resolve({
      status: 200,
      message: `You will receive PEPE in your wallet soon.`,
    })
  })
}
...
export default async function handler(req, res) {
  ...
  if (verified) {
    ...
    if (usedBridge) {
      const pepeBalance = parseFloat(web3.utils.fromWei(await web3.eth.getBalance(address)))
      if (pepeBalance < 0.001) {
        ...
        res.status(200).json({
          status: 403,
          message: 'Your current PEPE balance is above the minimum requirement to use the faucet.',
        })
      }
    } else {
      res.status(200).json({
