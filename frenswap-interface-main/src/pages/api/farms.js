...
import distributorAbi from '../../constants/abis/fren-distributor.json'
import pairAbi from '../../constants/abis/uniswap-v2-pair.json'
import { POOLS } from '../../constants/farms'
import { ChainId } from '../../sdk'

const NETWORK_URL = 'https://sepolia-rpc.pepechain.biz4'
...
export async function farms() {
  if (!farmsResult) {
    ...
    const ret = []
    const poolStaticInfo = POOLS[ChainId.PEPECHAIN_SEPOLIA]
    ...
    for (const pool of poolsInfo) {
      ...
      if (!staticInfo.token1) {
        ret.push({
          ...
          baseSymbol: staticInfo.token0.symbol.toLowerCase().replace('wpepe', 'pepe'),
        ...
        ret.push({
          ...
          baseSymbol:
            token0info.symbol == 'FREN' || token1info.symbol == 'FREN'
              ? 'fren'
              : token0info.symbol == 'PEPE' || token1info.symbol == 'PEPE'
              ? 'pepe'
              : token0info.symbol == 'USDC' || token1info.symbol == 'USDC'
              ? 'usdc'
              : '',
          baseAmount:
            token0info.symbol == 'FREN' || token0info.symbol == 'PEPE'
              ? token0amount
              : token1info.symbol == 'FREN' || token1info.symbol == 'PEPE'
              ? token1amount
