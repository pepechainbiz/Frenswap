...
import { usePositions, useFrenVaultInfo, useVaults } from '../../features/vault/hooks'
...
import { FREN_ADDRESS, AVERAGE_BLOCK_TIME, WNATIVE } from '../../constants'
import { VAULTS } from '../../constants/vaults'
import FrenswapLogo from '../../components/FrenswapLogo'
...
export default function Vault(): JSX.Element {
  ...
  const distributorInfo = useFrenVaultInfo()

  const priceData = useContext(PriceContext)

  const frenPrice = priceData?.['fren']
  const pepePrice = priceData?.['pepe']
  ...
  const summTvlVaults = vaults.reduce((previousValue, currentValue) => {
    return previousValue + (currentValue.totalLp / 1e18) * frenPrice
  }, 0)
  ...
  const map = (pool) => {
    ...
    function getRewards() {
      const rewardPerBlock =
        ((pool.allocPoint / distributorInfo.totalAllocPoint) * distributorInfo.frenPerBlock) / 1e18

      const defaultReward = {
        token: 'FREN',
        icon: '/images/token/fren.png',
        rewardPerBlock,
        rewardPerDay: rewardPerBlock * blocksPerDay,
        rewardPrice: frenPrice,
      }
    ...
    function getTvl(pool) {
      let lpPrice = 0
      let decimals = 18
      if (pool.lpToken == FREN_ADDRESS[chainId]) {
        lpPrice = frenPrice
  ...
  const valueStaked = positions.reduce((previousValue, currentValue) => {
    return previousValue + (currentValue.amount / 1e18) * frenPrice
  }, 0)

  return (
    <>
      <Head>
        <title>Vaults | Frenswap</title>
        <meta key="description" name="description" content="Fren Vaults" />
      </Head>
                  ...
                  <div className={`col-span-12 md:col-span-3 space-y-4`}>
                    <div className={`hidden md:block`}>
                      <div className={`col-span-12 md:col-span-4 bg-dark-800 px-6 py-4 rounded`}>
                        <div className="mb-2 text-2xl text-emphesis">{i18n._(t`Fren Vault`)}</div>
                        <div className="mb-4 text-base text-secondary">
                          <p>
                            {i18n._(
                              t`Fren Vault is a set of high incentivized pools. Long term supporters can choose to lock FREN for a determined period for higher rewards.`
                            )}
                          </p>
                          <p className="mt-2">
                            {i18n._(
                              t`The participants receive various benefits such as higher rewards according to lock duration, higher allocations in Fren Launchpad and more.`
                            )}
                          </p>
                        </div>
