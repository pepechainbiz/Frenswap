...
export default function Farm(): JSX.Element {
  ...
  const FILTER = {
    ...
    fren: (farm) => farm.pair.token0?.id == FREN_ADDRESS[chainId] || farm.pair.token1?.id == FREN_ADDRESS[chainId],
    single: (farm) => !farm.pair.token1,
    pepechain_sepolia: (farm) => farm.pair.token0?.id == WNATIVE[chainId] || farm.pair.token1?.id == WNATIVE[chainId],
    stables: (farm) =>
  ...
  const allStaked = positions.reduce((previousValue, currentValue) => {
    return previousValue + (currentValue.pendingFren / 1e18) * frenPrice
  }, 0)
  ...
  return (
    <>
      <Head>
        <title>Farm | Frenswap</title>
        <meta key="description" name="description" content="Farm FREN" />
      </Head>
