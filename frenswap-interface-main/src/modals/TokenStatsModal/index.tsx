...
import { useFrenContract } from '../../hooks'
...
export default function TokenStatsModal({ token }: { token: any }) {
  ...
  let tokenInfo = useTokenInfo(useFrenContract())

  if (token.symbol == 'PEPE') tokenInfo = { circulatingSupply: '420690000000000', burnt: '0', totalSupply: '420690000000000', vaults: '0' }

  const price = formatNumberScale(priceData?.[token.symbol.toLowerCase()], true, 2)

  const modalOpen = useModalOpen(token.symbol == 'FREN' ? ApplicationModal.FREN_STATS : ApplicationModal.PEPE_STATS)
            ...
            <div className="flex flex-1 flex-col">
              ...
              <div className="flex items-center justify-between space-x-3 gap-2">
                {token?.address && (
                  <ExternalLink
                    href={
                      'https://sepolia.pepechain.biz/tokens/0x6bD193Ee6D2104F14F94E2cA6efefae561A4334B'
                    }
