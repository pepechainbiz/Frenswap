...
import { usePendingFren, useUserInfo } from './hooks'
import Button from '../../components/Button'
import Dots from '../../components/Dots'
import { FREN_DISTRIBUTOR_ADDRESS, FREN_VAULT_ADDRESS } from '../../constants/addresses'
...
const VaultListItem = ({ farm }) => {
  ...
  const pendingFren = usePendingFren(farm)
  ...
  const [approvalState, approve] = useApproveCallback(typedDepositValue, FREN_VAULT_ADDRESS[chainId])
    ...
    <>
      <Modal isOpen={showConfirmation} onDismiss={() => setShowConfirmation(false)}>
        <div className="space-y-4">
          ...
          <Typography variant="lg" className="font-medium pt-4">
            {i18n._(
              t`${
                currentAction.action == 'deposit' ? 'Staking on Fren Vault' : 'Harvesting'
      ...
      >
        <Disclosure.Panel className="flex flex-col w-full border-t-0 rounded rounded-t-none bg-dark-800" static>
                ...
                <Button
                  ...
                  onClick={async () => {
                    const fn = async () => {
                      setPendingTx(true)
                      try {
                        // KMP decimals depend on asset, FLP is always 18
          ...
          {pendingFren && pendingFren.greaterThan(ZERO) && (
            <div className="px-4 pb-4">
              ...
              >
                {i18n._(t`Harvest ${formatNumber(pendingFren.toFixed(18))} FREN`)}
              </Button>
            </div>
          )}
        </Disclosure.Panel>
