...
export function useUserInfo(farm, token) {
  ...
  const contract = useFrenVaultContract()
...
export function useFrenPositions(contract?: Contract | null) {
  ...
  return useMemo(() => {
    if (!pendingFren || !userInfo || !userLockedUntil) {
      return []
    }
    return zip(pendingFren, userInfo, userLockedUntil)
      .map((data, i) => ({
        id: args[i][0],
  ...
  }, [args, pendingFren, userInfo, userLockedUntil])
}

export function usePositions() {
  return useFrenPositions(useFrenVaultContract())
}

export function useFrenVaults(contract?: Contract | null) {
  const numberOfPools = useSingleCallResult(contract ? contract : null, 'poolLength', undefined, NEVER_RELOAD)
    ?.result?.[0]
...
export function useVaults() {
  return useFrenVaults(useFrenVaultContract())
}
...
export function useFrenVaultInfo() {
  return useVaultInfo(useFrenVaultContract())
}
